...
class cEHRNerProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        return ["O", "B_疾病和诊断", "I_疾病和诊断", "B_药物", "I_药物", "B_手术", "I_手术","B_影像检查","I_影像检查","B_实验室检验","I_实验室检验","B_症状","I_症状","B_解剖部位","I_解剖部位","[CLS]","[SEP]"]
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        tmp_path = os.path.join(FLAGS.output_dir, "label2id.pkl")
        with tf.gfile.GFile(tmp_path,mode='rb') as rf:
